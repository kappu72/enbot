...
export class EnBot {
  ...
  private initializeCommands(supabase: SupabaseClient): void {
    ...
    this.commandRegistry.registerCommand(new TransactionCommand({
      supabase,
    ...
    this.commandRegistry.registerCommand(new QuoteCommand({
      supabase,
      telegram: this.telegram,
      sessionManager: this.commandRegistry['sessionManager'],
      userId: 0, // Will be set per request
  ...
  private async handleLegacyMessage(msg: TelegramUpdate['message']): Promise<void> {
    if (!msg?.text) return;
